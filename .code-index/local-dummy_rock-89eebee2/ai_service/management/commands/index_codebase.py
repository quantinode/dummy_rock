...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        tools = codemunch._get_tools()
        if 'index_folder' not in tools:
            self.stderr.write(self.style.ERROR(
                'jCodeMunch tools not available. Please install or update jcodemunch_mcp:\n'
                '  pip install --upgrade jcodemunch_mcp\n'
                'Or skip code indexing - this feature is optional.'
            ))
            return
        ...
        try:
            ...
            self.stderr.write(self.style.WARNING(
                '\nCode indexing is optional. You can still use the platform without it.'
            ))
